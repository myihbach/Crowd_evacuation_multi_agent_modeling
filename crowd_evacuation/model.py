...
class FireEvacuation(Model):
    ...
    def __init__(self, floor_plan_file, human_count, collaboration_percentage, fire_probability, visualise_vision, random_spawn, save_plots):
        # Load floorplan
        # floorplan = np.genfromtxt(path.join("evacuation_foule/plan_sol/", floor_plan_file))
        with open(os.path.join("evacuation_foule/plan_sol/", floor_plan_file), "rt") as f:
        ...
        self.datacollector = DataCollector(
            {
                "Vivant": lambda m: self.count_human_status(m, Human.Status.ALIVE),
                "Mort": lambda m: self.count_human_status(m, Human.Status.DEAD),
                "Echappé": lambda m: self.count_human_status(m, Human.Status.ESCAPED),
                "Incapable": lambda m: self.count_human_mobility(m, Human.Mobility.INCAPACITATED),
                "Normal": lambda m: self.count_human_mobility(m, Human.Mobility.NORMAL),
                "Paniqué": lambda m: self.count_human_mobility(m, Human.Mobility.PANIC),
                "Collaboration verbale": lambda m: self.count_human_collaboration(m, Human.Action.VERBAL_SUPPORT),
                "Collaboration physique": lambda m: self.count_human_collaboration(m, Human.Action.PHYSICAL_SUPPORT),
                "Collaboration morale": lambda m: self.count_human_collaboration(m, Human.Action.MORALE_SUPPORT)
            }
        )
    ...
    def save_figures(self):
        ...
        status_results = results.loc[:, ['Vivant', 'Mort', 'Echappé']]
        status_plot = status_results.plot(ax=axes[0])
        status_plot.set_title("Statut des agents")
        status_plot.set_xlabel("Simulation Step")
        status_plot.set_ylabel("Count")

        mobility_results = results.loc[:, ['Incapable', 'Normal', 'Paniqué']]
        mobility_plot = mobility_results.plot(ax=axes[1])
        mobility_plot.set_title("Mobilité des agents")
        mobility_plot.set_xlabel("Simulation Step")
        mobility_plot.set_ylabel("Count")

        collaboration_results = results.loc[:, ['Collaboration verbale', 'Collaboration physique', 'Collaboration morale']]
        collaboration_plot = collaboration_results.plot(ax=axes[2])
        collaboration_plot.set_title("Collaboration des agents")
        ...
        plt.suptitle("Pourcentage de collaboration : " + str(self.collaboration_percentage) + "%, Nombre des agents: " + str(self.human_count), fontsize=16)
        plt.savefig(OUTPUT_DIR + "/figures/" + timestr + ".png")
        plt.close(fig)
